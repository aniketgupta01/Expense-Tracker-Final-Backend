...
async function submitExpense(e){
    ...
    try{
        ...
        let result = await axios.post('http://13.51.158.192:6500/expense/add-expense',obj,{headers:{'Authorization':token}});
...
window.addEventListener('DOMContentLoaded',async() => {
    ...
    let expenses = await axios.get(`http://13.51.158.192:6500/expense/get-expenses?page=${page}&&limit=${limit}`,{headers:{"Authorization":token}});
...
async function deleteExpense(id){
    const token = localStorage.getItem('token')
    let result = await axios.get(`http://13.51.158.192:6500/expense/delete-expense/${id}`,{headers : {"Authorization" : token}});
...
document.getElementById('rzp-button').onclick = async function (e){
    e.preventDefault();
    const token = localStorage.getItem('token');
    const response = await axios.get('http://13.51.158.192:6500/purchase/premiumMembership',{headers : {"Authorization" : token}})
    ...
    var options = {
        ...
        "handler": async function(response){
            await axios.post('http://13.51.158.192:6500/purchase/updatetransactionstatus',{
...
async function showLeaderboard(){
    const leaderboardButton = document.getElementById('leaderboard-button');
            const token = localStorage.getItem('token');
            const userLeaderboardArray = await axios.get('http://13.51.158.192:6500/premium/showLeaderboard',{headers:{
                'Authorization':token
            }})
