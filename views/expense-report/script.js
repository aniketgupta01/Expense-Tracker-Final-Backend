...
const expense = await axios.get('http://13.51.158.192:6500/expense/get-expenses',{headers:{'Authorization':token}});
...
async function downloadReport(){
    const token = localStorage.getItem('token');
    try{
    const result = await axios.get('http://13.51.158.192:6500/expense/download',{headers:{'Authorization':token}})
